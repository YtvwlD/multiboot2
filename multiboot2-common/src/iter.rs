...
/// Iterates mutably over the tags (modelled by [`DynSizedStructure`]) of the
/// underlying byte slice. Each tag is expected to have the same common [`Header`].
///
/// As the iterator emits elements of type [`DynSizedStructure`], users should
/// casted them to specific [`Tag`]s using [`DynSizedStructure::cast`] following
/// a user policy. This can for example happen on the basis of some ID.
...
/// [`Tag`]: crate::Tag
#[derive(Debug)]
pub struct TagIterMut<'a, H: Header> {
    /// Absolute offset to next tag and updated in each iteration.
    next_tag_offset: usize,
    buffer: &'a mut [u8],
    // Ensure that all instances are bound to a specific `Header`.
...
impl<'a, H: Header> TagIterMut<'a, H> {
    /// Creates a new iterator.
    #[must_use]
    pub fn new(mem: &'a mut [u8]) -> Self {
        // Assert alignment.
        assert_eq!(mem.as_mut_ptr().align_offset(ALIGNMENT), 0);
...
impl<'a, H: Header + 'a> Iterator for TagIterMut<'a, H> {
    type Item = &'a mut DynSizedStructure<H>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let ptr = unsafe { self.buffer.as_mut_ptr().add(self.next_tag_offset) }.cast::<H>();
        let tag_hdr = unsafe { &mut *ptr };
        ...
        let slice = {
            ...
            &mut self.buffer[from..to]
        };

        // unwrap: We should not fail at this point.
        // TODO: this was ref_from_slice_mut, but the borrow checker doesn't want this
        let tag = unsafe { DynSizedStructure::ref_from_ptr_mut(NonNull::new(
            slice.as_mut_ptr().cast::<H>()
        ).unwrap()).unwrap() };
        Some(tag)
    }
}
